...
CFG.performFunction = function(fnc, msg) {
	...
	CFG.timeoutId = window.setTimeout(function() {
		...
		var gifPath = (ipAddress !== "" ? document.location.protocol + "//" + ipAddress + "/" : "") +
...
CFG.lookup = function(msgObj) {
	var result = EMPTY;
	var nodes = $(CFG.xml).find("Messages");
	$(nodes).each(
		function() {
...
CFG.setLabelsAndTitles = function(xml) {
	...
	var nodes = $(xml).find("Controls").children();
	$(nodes).each(
		function() {
			...
			switch (type) {
				...
				case "caption":
					...
					break;
				case "link":
					var pfx = sel.hasClass("abt_lnkExpanded") ? "-" : "+";
					sel.text(pfx + lbl);
					sel.attr("title", ttp);
					break;
				case "column":
					if (!cols) break;
					var clm = cols[grd.getColumnIndex($(this).attr("Id"))];
					if (!clm) break;
					clm.name = lbl;
					clm.toolTip = ttp;
					break;
...
CFG.loadLanguageCombo = function() {
	...
	if (CMN.isMockMode()) {
		...
		var xml = CMN.getXml("/services/network/languagedata?lang=" + defaultLanguage);
...
CFG.docSize = function(w, h) {
	if (typeof w == "string" || typeof w == "boolean") {
		var z = CFG.defaultDocSize(typeof w == "boolean");
		w = z.width;
		h = z.height;
...
CFG.defaultDocSize = function(useOptimal) {
	var w = (CFG.getLanguage() === "en") ? 1008 : 1135;
	var h = 0;
