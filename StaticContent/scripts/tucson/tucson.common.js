...
CMN.getUserLevel = function() {
	const ul = CMN.getCookie(document, "userlevel");
...
CMN.getRemember = function() {
	const ul = CMN.getCookie(document, "remember");
	return (ul) ? ul : false;
};
...
CMN.isLoggedIn = function() {
	const ul = CMN.getCookie(document, "key");
	return !!ul;
};
...
CMN.isXmlEqual = function(xmlDoc1, xmlDoc2) {
	const pat = /^<\?xml\s+version\s*=\s*(["'])[^\1]+\1[^?]*\?>/;
...
CMN.setCookie = function(doc, cname, value, exdays) {
	...
	const cvalue = window.escape(value) + ((exdays === null) ? "" : "; Expires=" + exdate.toUTCString()); // Path=/;
 ...
 * @param {String} title Title displayed in the dialog header (default = document.title).
 * @param {Number} width Width of the dialog (default = 450).
 * @param {Number} height Height of the dialog (default = 200).
 */
...
CMN.showConfirm = function(message, title, icn, btn1, btn2, width, height) {
	...
	const htm = (icn) ? '<span class="dialog_image"><img src="' + CMN.TUCSON_IMAGE_PATH + icn + '"></span>' + message : message;
...
CMN.openWindow = function(winUrl, winName, winFeatures, winObj) {
	...
	const theWin = window.open(winUrl, winName, winFeatures);
	return theWin;
};
...
CMN.checkForError = function() {
	const error = CMN.getCookie(document, "error");
	if (error) {
